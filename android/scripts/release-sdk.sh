...
echo "Releasing Veksti Meet SDK ${SDK_VERSION}"
...
# Now build and publish the Veksti Meet SDK and its dependencies
echo "Building and publishing the Veksti Meet SDK"
pushd ${THIS_DIR}/../
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Veksti Meet SDK + dependencies: ${SDK_VERSION}"
    popd
